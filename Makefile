...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=lotr_movie_sdk tests/
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 lotr_movie_sdk tests
...
check-safety:
	...
	poetry run bandit -ll --recursive lotr_movie_sdk tests
